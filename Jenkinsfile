pipeline {
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                // If this job is "Pipeline script from SCM", use:
                checkout scm

                // OR if this is a plain "Pipeline script" job, comment the line above
                // and uncomment this with your real repo URL + branch:
                // git branch: 'main',
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'dockerpass', toolName: 'docker') {
                        // 🔹 RELATIVE PATH INSIDE WORKSPACE, NOT /var/lib/...
                        dir('src/adservice') {
                            sh 'ls -la' // debug: confirm Dockerfile is here
        ...
        stage('EKS-Deployment') {
            steps {
                withKubeConfig(
                    ...
                    serverUrl: 'https://80D4B7704D7C7CB7C8B578EEA161B873.gr7.us-east-1.eks.amazonaws.com'
                ) {
                    // Make sure these YAMLs exist in the workspace (e.g., at repo root or a k8s/ folder)
